...
class User(UserMixin, db.Model):
    ...
    talks = db.relationship('Talk', lazy='dynamic', backref='author')  # lazy makes talks query not list
    ...
    @property
    def password(self):  # made it pprty so that i can say user.password=? so that it looks like a member var
    ...
    def verify_password(self, password):  # called when user logs in
        return check_password_hash(self.password_hash,
                                   password)  # takes plain text,calls werks function,applies trans fnt to plain txt,compares the hashes
...
class Comment(db.Model):
    ...
    def on_changed_body(target, value, oldvalue, initiator):  # funtion takes value,html
        allowed_tags = ['a', 'abbr', 'acronym', 'b', 'blockquote', 'code',
                        ...
                        'h1', 'h2', 'h3', 'p']  # uses bleach to filter out tags not specified
        target.body_html = bleach.linkify(bleach.clean(
            markdown(value, output_format='html'),
            tags=allowed_tags, strip=True))  # stores it here
